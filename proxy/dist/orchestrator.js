...
function sessionFilePath(channel) {
    return join(SESSION_DIR, `${channel}.json`);
}
function saveSession(channel, sessionId) {
...
function mapCliMessageToEvents(msg, channel, relay, batcher) {
    const type = msg.type;
    switch (type) {
        // --- System init ---
        case "system": {
...
function writeAttachments(channel, attachments) {
...
function sendPromptToProcess(child, text, attachments) {
...
async function main() {
    ...
    rl.on("line", (line) => {
        ...
        // Log turn completion
        if (msg.type === "result") {
            log(`Turn complete. Cost: $${msg.total_cost_usd?.toFixed(4)}`);
        }
    ...
    relay.onMessage((msg) => {
        ...
        else if (msg.type === "interrupt_request" && msg.channel === channel) {
